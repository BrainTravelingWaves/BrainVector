function [ Rec ] = meschm_emfield( cortexfile,inner,head,outer,channel,iEeg,dipe, elem,field,Gain)
...
elseif field=='m'
        OPTIONS.MEGMethod='openmeeg';
        OPTIONS.EEGMethod='';
        OPTIONS.iMeg=iEeg;  
        OPTIONS.BemCond=[1 0.0025 1];
        OPTIONS.BemSelect=[1 1 1];
end
OPTIONS.ECOGMethod='';
OPTIONS.SEEGMethod='';
...
G=bst_openmeeg(OPTIONS);
for i=1:size(dipe.Loc,1)
Rec(:,i)=G*reshape((squeeze(dipe.elem.Amp(i,:,:)))',size(G,2), 1);
end
end
end

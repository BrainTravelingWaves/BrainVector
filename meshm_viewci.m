...
function [] = meshm_viewci(mesh,Amp,dipe,lendip,right,vert_right,views,al,video) % показ волны в левом полушарии
  if right==0
     [Vertices, Faces] = tess_remove_vert(mesh.Vertices, mesh.Faces, [(vert_right+1):numel(mesh.Vertices(:,1))]);
     Amp((vert_right+1):numel(mesh.Vertices(:,1)),:)=[]; % Убираем правое полушарие
  end
  if right==1
     [Vertices, Faces] = tess_remove_vert(mesh.Vertices, mesh.Faces, [1:vert_right]); % убирать часть значений в переменной mesh_dist ???
      Amp(1:(vert_right),:)=[];  
  end
  if right > 1
